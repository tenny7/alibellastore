...
## 10. Code touchpoints in Alibella Stores

- `src/lib/momo/client.ts` — token, requestToPay (EUR-in-sandbox, callback-in-prod), status
- `src/app/api/payments/momo/initiate/route.ts` — starts a MoMo payment
- `src/app/api/payments/momo/status/[referenceId]/route.ts` — polls + confirms on SUCCESSFUL
- `src/app/api/payments/momo/callback/route.ts` — async callback (production only)
- `src/app/api/payments/momo/confirm/route.ts` — manual confirmation
- `src/app/api/payments/cod/route.ts` — Cash-on-Delivery (no gateway)
- `scripts/momo-setup.mjs` — provisioning + env bootstrap
- `scripts/test-momo-sandbox.mjs` — sandbox smoke test / UAT evidence
- `src/app/admin/settings/payments` — payment configuration status
- `src/app/admin/orders/[id]` — shows MoMo reason + transaction id for support

Note: this app's callback path is `/api/payments/momo/callback`, not
`/api/webhooks/momo`. `MOMO_CALLBACK_URL` and the API user's
`providerCallbackHost` are both set to `alibellastore.com`, so they match.

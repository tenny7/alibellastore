...
export default async function AdminInventoryPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-display text-[28px] font-bold tracking-[-0.03em] text-cream">Inventory</h1>
      ...
      <div className="mb-8 flex gap-3 rounded-[18px] border border-cream/[0.12] bg-cream/[0.04] p-4">
        ...
        <p className="text-[13px] leading-relaxed text-cream/60">
          ...
          to <code className="font-mono text-[12px] text-cream/80">products</code> would let this page
      ...
      {outOfStock.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-3 font-display text-[11px] font-medium uppercase tracking-[0.14em] text-cream/45">
            Needs attention
          </h2>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {outOfStock.map((p) => (
              <Link
                ...
                className="flex items-center gap-3 rounded-[18px] border border-cream/[0.09] bg-ink p-3 transition-colors hover:border-cream/20"
              >
                <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-[10px] bg-cream/[0.06]">
                  {p.images?.[0] && (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 truncate text-[12px] text-cream/45">
      ...
      <section>
        ...
        {products.length === 0 && (
          <p className="rounded-[18px] border border-cream/[0.09] bg-ink px-6 py-12 text-center text-cream/45">
            No products yet.
          </p>
        )}
      </section>

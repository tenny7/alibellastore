...
export default async function AdminDispatchPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-display text-[28px] font-bold tracking-[-0.03em] text-cream">Dispatch board</h1>
            ...
            >
              <header className="flex items-center gap-2.5 border-b border-cream/[0.09] px-4 py-3.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-display text-sm font-bold text-cream">{lane.title}</div>
                  <div className="text-[11px] text-cream/40">{lane.hint}</div>
                </div>
                <span className="rounded-md bg-cream/[0.08] px-2 py-1 font-mono text-[11.5px] font-medium text-cream/70">
                  {items.length}
                </span>
              </header>
              ...
              <div className="flex flex-1 flex-col gap-2 p-3">
                {items.length === 0 && (
                  <p className="px-1 py-6 text-center text-xs text-cream/30">Nothing here</p>
                )}
                {items.map((o) => (
                  <Link
                    key={o.id}
                  ...
                  >
                    ...
                    <div className="mt-2 truncate text-[13px] font-medium text-cream">{o.customer_name}</div>
                    <div className="mt-0.5 truncate font-mono text-[11.5px] text-cream/45">{o.customer_phone}</div>
                    <div className="mt-2 line-clamp-2 text-[11.5px] leading-snug text-cream/40">
                      {o.shipping_address}
                    </div>
                  </Link>
                ))}

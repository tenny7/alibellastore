/**
 * MTN MoMo Collection — portable setup / bootstrap (integration kit).
 *
 * Run this FROM your target app's directory. It:
 *   1. provisions a sandbox API user + API key from your Primary Key,
 *   2. creates .env.local (or merges MOMO_* into an existing one — never
 *      clobbering your other variables),
 *   3. optionally scaffolds lib/momo/{client.ts,types.ts} into the app,
 *   4. smoke-tests token → requesttopay → status.
 *
 ...
 *
 * Usage (from inside the app you're integrating):
 *   MOMO_COLLECTION_PRIMARY_KEY=<key> node /path/to/momo-integration-kit/momo-setup.mjs --scaffold
 *   node /path/to/.../momo-setup.mjs --key=<key> --scaffold=src/lib/momo
 *   node /path/to/.../momo-setup.mjs --key=<key> --no-test
 *   node /path/to/.../momo-setup.mjs --env=production --host=https://proxy.momoapi.mtn.com --key=<prodkey>
 *
 * Flags: --key --env(sandbox|production) --host --callback --currency --file
 *        --reprovision --no-test --scaffold[=<dir>] --force
 */
...
// ── env file (in the TARGET app = cwd) ──────────────────────────────────────
...
const PRIMARY = flags.key || cfg("MOMO_COLLECTION_PRIMARY_KEY")
if (!PRIMARY || /^your-|^<|placeholder/i.test(PRIMARY)) {
  die(
    "No valid MOMO_COLLECTION_PRIMARY_KEY.\n" +
      ...
      "  Primary Key), then re-run with --key=<key>.",
  )
}
...
if (haveCreds && !flags.reprovision) {
  ...
  apiUser = randomUUID()
  {
    const url = `${HOST}/v1_0/apiuser` // host root, NOT /collection
...
if (toAppend.length) {
  ...
  lines.push("# MTN MoMo (Collection) — added by momo-setup.mjs")
  for (const k of toAppend) lines.push(`${k}=${updates[k]}`)
}
writeFileSync(ENV_PATH, lines.join("\n").replace(/\n*$/, "\n"))
log(`\n✓ Wrote ${ENV_PATH}`)
log(`  updated: ${[...seen].join(", ") || "(none)"}`)
log(`  added:   ${toAppend.join(", ") || "(none)"}`)
...
log("\n\x1b[32mDone.\x1b[0m")
log("Credentials are in .env.local. Restart `npm run dev` to pick them up.")
log("Smoke-test any time with: node scripts/test-momo-sandbox.mjs")
